...
if object_id(N'[math_test].[divide]'
             , N'P') is not null
...
/*
	...
	------------------------------------------------
*/
create procedure [math_test].[divide] @stack [xml] output
as
  begin
...
as
  begin
      return round(cast (@numerator as [float]) / cast(@denominator as [float])
                   , 6);
  end;
...
as
  begin
      ...
      if @numerator is null
        ...
        set @return = round(cast (@numerator as [float]) / cast(@denominator as [float])
                            , 6);
...
--
-------------------------------------------------
if object_id(N'[math_test].[divide_using_decimal]'
             , N'P') is not null
...
*/
create procedure [math_test].[divide_using_decimal] @stack [xml] output
as
  begin
      declare @test_stack     [xml] = N'<test_stack fqn="" test_count="0" error_count="0" pass_count="0" />',
              @test_prototype [xml] = N'<test test_sequence="0" test_name="{replace_me}" expected="pass" actual="fail" />',
      ...
      if object_id(N'[math].[divide]'
        ...
        begin
            ...
            begin
                ...
                begin try
                    set @test_value = [math].[divide](33.333333
                                                      , 3)
...
create function [math].[divide] (@numerator     [decimal](10, 6)
                                 , @denominator [int])
returns [decimal](10, 6)
as
  begin
...
*/
create procedure [math_test].[run] @stack [xml] output
as
  begin
...
*/
create procedure [test].[run] @stack [xml] output
as
  begin
      declare @test_stack          [xml],
              @test_suite          [xml] = N'<test_suite stack_count="0" test_count="0" error_count="0" pass_count="0" />',
...
select @stack
       , @stack.value(N'(/*/@stack_count)[1]'
                      , N'[int]') as [stack_count]
       , @stack.value(N'(/*/@test_count)[1]'
                      , N'[int]') as [test_count]
       , @stack.value(N'(/*/@pass_count)[1]'
                      , N'[int]') as [pass_count]
       , @stack.value(N'(/*/@error_count)[1]'
                      , N'[int]') as [error_count];
  ...
  begin
      ...
      if object_id(N'[test].[run]'
        ...
        begin
            ...
            set @error_count = @stack.value(N'(/*/@error_count)[1]'
                                            , N'[int]')
...
--
if object_id(N'[math_secure].[data]'
             , N'U') is not null
...
-- code block begin
-------------------------------------------------
if object_id(N'[math_test].[set]'
             , N'P') is not null
  drop procedure [math_test].[set];

go

create procedure [math_test].[set] @stack [xml] = null output
as
  begin
