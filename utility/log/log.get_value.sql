...
if object_id(N'[log].[get_value]'
             , N'FN') is not null
  drop function [log].[get_value];
...
/*
	...
	------------------------------------------------
	declare @schema [sysname] = N'log', @object [sysname] = N'get_value';
	--
	-------------------------------------------------
...
*/
create function [log].[get_value] (@id          [uniqueidentifier]
                                   , @category  [sysname]
                                   , @class     [sysname]
                                   , @type      [sysname]
                                   , @timestamp [datetime])
...
exec sys.sp_addextendedproperty
  @name = N'execute_as',
  @value = N'
	select [log].[get_value] (N''category'', N''class'', N''type'', null);',
  @level0type = N'schema',
  @level0name = N'log',
  @level1type = N'function',
  @level1name = N'get_value';
