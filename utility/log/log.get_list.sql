...
if object_id(N'[log].[get_list]'
             , N'TF') is not null
  drop function [log].[get_list];
...
/*
	...
	------------------------------------------------
	declare @schema [sysname] = N'log', @object [sysname] = N'get_list';
...
*/
create function [log].[get_list] (@category    [sysname]
                                  , @class     [sysname]
                                  , @type      [sysname]
                                  , @timestamp [datetime])
returns @list table (
...
as
  begin
      ...
      select [id]
             ...
             , [entry]
             , [created]
      from   [repository_secure].[get_list] (N'log'
                                             , @category
                                             , @class
                                             , @type
                                             , @timestamp);
...
exec sys.sp_addextendedproperty
  @name = N'execute_as',
  @value = N'
		select * from [log].[get_list] (N''clinicScorecard'', N''patientExperience'', N''cfsParticipation'');',
  @level0type = N'schema',
  @level0name = N'log',
  @level1type = N'function',
  @level1name = N'get_list';
