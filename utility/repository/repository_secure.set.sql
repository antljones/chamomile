...
/*
	...
	------------------------------------------------
	declare @schema [sysname] = N'repository_secure', @object [sysname] = N'set';
	--
	-------------------------------------------------
...
*/
create procedure [repository_secure].[set] @source                 [sysname]
                                           , @category             [sysname]
                                           , @class                [sysname]
                                           , @type                 [sysname]
                                           , @entry                [xml]
...
as
  begin
      ...
      select @active = coalesce(@active
                                , @current_timestamp)
             , @expire_timestamp = dateadd(second
                                           , -1
                                           , @current_timestamp);
      ...
      if @immutable = 1
        begin
            if @expire_prior_entries = 1
              begin
                  update [repository_secure].[immutable]
        ...
        begin
            if @expire_prior_entries = 1
              begin
                  update [repository_secure].[mutable]
                  set    [expire] = @expire_timestamp
                  where  [source] = @source
                         and [category] = @category
                         and [class] = @class
            ...
            insert into [repository_secure].[mutable]
                        ([source],
                         [category],
                         [class],
                         [type],
...
exec sys.sp_addextendedproperty
  @name = N'description',
  @value = N'Primary mutator method for [repository_secure_secure].[repository_secure]. Only an insert is provided for. In this manner the repository_secure trail is maintained.',
  @level0type = N'schema',
  @level0name = N'repository_secure',
  @level1type = N'procedure',
  @level1name = N'set'
...
------------------------------------------------- 
if exists (select *
           from   fn_listextendedproperty(N'package_repository_secure'
                                          , N'schema'
                                          , N'repository_secure'
                                          , N'procedure'
                                          , N'set'
                                          ...
                                          , default))
  exec sys.sp_dropextendedproperty
    @name = N'package_repository_secure',
    @level0type = N'schema',
    @level0name = N'repository_secure',
    @level1type = N'procedure',
    @level1name = N'set'
